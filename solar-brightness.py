...
def display_get(idx, retry=True, last_val=None):
    """
    读取显示器亮度。如果与上次记录偏差过大（>30%），
    可能是唤醒瞬断虚值，自动重试验证。
    """
    try:
        ...
        # 异常检测：与上次记录偏差 >30% 且值是 0 或 100，视为可疑
        if retry and last_val is not None and val in (0, 100) and abs(val - last_val) > 30:
            log.debug("  亮度 %d%% 与上次 %d%% 偏差过大，疑似虚值，1s 后重试...", val, last_val)
            time.sleep(1)
...
def acquire_lock():
    """
    POSIX 文件锁，防并发执行。
    用 fcntl.flock (BSD) 替代 lockf，macOS 上更可靠。
    """
    fp = open(LOCK_FILE, "w")
    try:
...
def run_once(cfg):
    ...
    for (ddc_idx, name, ident) in displays:
        ...
        display_key = ident or name
        last_val = state.get(display_key, {}).get("brightness")
        cur = display_get(ddc_idx, last_val=last_val)
        if cur is None:
        ...
        if display_set(ddc_idx, new_val):
            ...
            state[display_key] = {"brightness": new_val, "timestamp": time.time()}
        else:
...
def cmd_install(anchors=None, enable_schedule=False):
    ...
    # 从配置读取间隔（默认 30 分钟）用于 plist
    tick_seconds = 1800
    if CONFIG_PATH.exists():
        try:
            with open(CONFIG_PATH) as f:
                c = yaml.safe_load(f)
            tick_seconds = c.get("global", {}).get("transition", {}).get("tick_minutes", 30) * 60
        except Exception:
            pass
